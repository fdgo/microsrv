...
|服务|命名空间|说明|---|
|---|---|---|---|
|接入层API|fred.micro.shop.web|负责代理所有**fred.micro.shop.web**下游的web应用，比如**fred.micro.shop.web.user**等|---|
|用户web|fred.micro.shop.web.user|接收API下放的路由为/user请求|---|
|用户服务|fred.micro.shop.srv.user|对架构内应用提供user查询服务|---|
...
```bash
micro new --namespace=fred.micro.shop --type=srv --alias=user github.com/wangmhgo/microservice-project/go-micro-learn/microservices/part1/user-srv
```

我们解释一下各个flag参数

- namespace，因为我们要让**web**直接暴露在API之下，而本篇后面我们会开一个handler模式为web的API，它的命名空间为`fred.micro.shop.web`，故而，我们服务
...
package fred.micro.shop.srv.user;
...
func main() {
    ...
    micro.RegisterSubscriber("fred.micro.shop.srv.user", service.Server(), new(subscriber.Service))

    // Register Function as Subscriber
    micro.RegisterSubscriber("fred.micro.shop.srv.user", service.Server(), subscriber.Service)
...
2019/04/12 23:57:12 [Init] 加载配置文件：path: /Users/me/workspace/go/src/github.com/wangmhgo/microservice-project/go-micro-learn/microservices/part1/user-srv/conf/application.yml, {Include:consul, db}
2019/04/12 23:57:12 [Init] 加载配置文件：path: /Users/me/workspace/go/src/github.com/wangmhgo/microservice-project/go-micro-learn/microservices/part1/user-srv/conf/application-consul.yml
2019/04/12 23:57:12 [Init] 加载配置文件：path: /Users/me/workspace/go/src/github.com/wangmhgo/microservice-project/go-micro-learn/microservices/part1/user-srv/conf/application-db.yml
2019/04/12 23:57:12 Transport [http] Listening on [::]:52801
2019/04/12 23:57:12 Broker [http] Connected to [::]:52802
2019/04/12 23:57:12 Registry [consul] Registering node: fred.micro.shop.srv.user-f1cb2a6c-1c8b-4d90-97b6-a9e287c1acc4
...
```bash
$ micro --registry=consul call fred.micro.shop.srv.user User.QueryUserByName '{"userName":"micro"}'
{
   "user": {
       "id": 10001,
       "name": "micro",
...
```bash
micro new --namespace=fred.micro.shop --type=web --alias=user github.com/wangmhgo/microservice-project/go-micro-learn/microservices/part1/user-web
```
...
func Init() {
    serviceClient = us.NewUserService("fred.micro.shop.srv.user", client.DefaultClient)
}
